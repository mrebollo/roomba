 ...
 */
const char *competition_get_team_id(void) { return get_team_id(); }
...
void run_competition(void) {
  ...
  char *maps[] = {"../maps/noobs.pgm", "../maps/random1.pgm",
                  "../maps/random3.pgm", "../maps/random5.pgm"};
  ...
  if (!fd) {
    ...
    fprintf(stderr,
            "Please create config.txt with: echo \"0 0\" > config.txt\n");
    exit(1);
  }
  ...
  char map_path[256];
  if (g_maps_dir) {
    snprintf(map_path, sizeof(map_path), "%s/%s", g_maps_dir,
             strrchr(maps[map_id], '/') + 1);
  } else {
...
void save_stats_competition(void) {
  ...
  fprintf(fd, "%s,%d,%d,%d,%d,%d,%.1f,%.1f,%d,%d,%d,%d,%d\n", team_id, map_type,
          stats.cell_total, stats.cell_visited, stats.dirt_total,
          stats.dirt_cleaned, stats.bat_total, stats.bat_mean, stats.moves[FWD],
          stats.moves[TURN], stats.moves[BUMP], stats.moves[CLEAN],
          stats.moves[LOAD]);
