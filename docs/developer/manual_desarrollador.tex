...
\begin{itemize}
    \item \textbf{Manual del Usuario:} \texttt{docs/user/manual\_usuario.pdf}
...
Para compilar en modo competición (sin visualización y con tiempo de ejecución fijo para todos los equipos):

\begin{verbatim}
gcc -DCOMPETITION_MODE=1 main.c simula.c ... -o roomba_comp
\end{verbatim}

Efectos:
\begin{itemize}
    \item \texttt{DEBUG\_PRINT()} se convierte en no-op
    \item La visualización se desactiva automáticamente
    \item El número de iteraciones (exec\_time) es el mismo para todos los equipos
