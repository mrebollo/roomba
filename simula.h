 ...
 *
 * El usuario debe crear una función de perfil void func(void) que contenga el
 * código de un ciclo del robot. Esta función la ejecuta hasta que alcanza el
 * tiempo de simulación o cualquier otra condición de terminación.
 *
 */
void run();
 ...
 * y la posición del robot en cada momento, dejando marcas sobre la trayectoria
 * que ha seguido. Una barra de progreso indica la carga de la batería y se
 * visualiza el estado del bumper (detección de obstáculos) y del sensor de
 * infrarrojos (detección de suciedad).
 */
void visualize();
 ...
 *
 * El robot dispone de una base en la que reposa cuando está inactivo. Para
 * activar al robot, debe ejecutarse esta acción awake() cuando se encuentre en
 * la posición de la base. La base siempre estará pegada a un muro y el ángulo
 * inicial del robot será perpendicular a él.
 *
 * El método devuelve a través de los parámetros las coordenadas x e y de la
 * base.
 *
...
/**
 * @brief Acción de girar el robot un ángulo 'alpha' respecto a la orientación
 * actual
 *
 ...
 *
 * Si detecta un obstáculo, se activarán los bumpers y no se cambiará la
 * posición. Esta acción consume:
 ...
 *
 * Cada ciclo de carga restaura 10 unidades de batería sin sobrepasar el máximo
 * (1000). Para poder ejecutar esta acción, el robot debe encontrarse en la
 * posición de la base.
 *
 * Esta acción no consume batería.
 *
